        ...
        <div className="absolute top-6 left-1/2 -translate-x-1/2 z-20">
          <div className="bg-white/95 backdrop-blur-sm px-6 py-4 rounded-xl shadow-lg flex items-center gap-3 border border-slate-100">
            ...
            <button
              ...
              className="ml-2 inline-flex items-center gap-2 text-xs px-3 py-2 rounded-lg bg-indigo-50 text-indigo-700 border border-indigo-100 hover:bg-indigo-100 transition"
              disabled={isLoading}
            >
            ...
            {filterButtons.map((btn) => (
              <button
                ...
                onClick={() => {
                  ...
                  if (btn.key === 'geofence' && onNavigate) onNavigate('vehicles');
                }}
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-lg font-semibold text-slate-900 flex items-center gap-2">
              <Activity className="h-4 w-4 text-indigo-600" /> {counts.total} vehicles
            </p>
          </div>
          <button
            onClick={fetchTelemetry}
          ...
          {filteredVehicles.map((vehicle) => (
            <button
              ...
              className={`w-full text-left p-3 rounded-lg border transition shadow-sm hover:shadow-md ${
                selectedVehicleId === vehicle.id ? 'border-indigo-200 bg-indigo-50' : 'border-slate-200 bg-white'
              }`}
            >
