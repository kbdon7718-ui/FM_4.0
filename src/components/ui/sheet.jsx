...
const SheetContent = React.forwardRef(({ className, children, side = 'right', hiddenTitle = 'Sheet', ...props }, ref) => {
  // create an id for an aria-describedby fallback
  const descId = `sheet-desc-${Math.random().toString(36).slice(2, 9)}`;
  ...
  return (
    <SheetPortal>
      ...
      >
        ...
        <div id={descId} className="sr-only">{hiddenTitle}</div>
        {children}
